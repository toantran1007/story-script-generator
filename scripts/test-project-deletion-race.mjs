...
async function scenario(ignoreAbort) {
  ...
  let resolveRequest, rejectRequest, confirmed = true, aborts = 0
  globalThis.window = { api: {
    ...
    confirmDeleteProject: async () => confirmed,
    ...
    deleteProject: async (id) => { trash.set(id, disk.get(id)); disk.delete(id); return [] },
  ...
  const project = { ...createEmptyProject('race', 'Race'), idea: 'A simple story', duration: 1, enableHook: false,
  ...
  confirmed = false
  await store.getState().deleteProject('race')
  assert(disk.has('race')); assert.equal(aborts, 0, 'canceling confirmation does not interrupt writing')
  confirmed = true
  ...
  console.log(`PASS: confirmed deletion waits/cancels owner, blocks late stream writes and restores a fresh generation (ignoreAbort=${ignoreAbort})`)
}
